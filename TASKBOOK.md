...
M4 is in progress: its first epic, M4-E1 (External Data Connectors), is
now complete; a second epic, M4-E2 (Delta REST/WS Completion &
Liquidation Heatmap Investigation), is not started — see `M4-E2-T1`/
`M4-E2-T2` below. M4-E1's first item, the reusable connector abstraction
...
M4-E2 (Delta REST/WS Completion & Liquidation Heatmap Investigation) is
not started. Confirmed directly against the real Delta client/parser
code before writing this down, rather than accepting a prior request's
"ticker, funding rate, open interest, mark price" list verbatim: three of
those four are already substantially built. Ticker is fully wired
(`LIVE_CHANNELS` in `app/runtime.py` subscribes it; `TickerEvent` is
parsed and published as `TickerUpdated`; reaches the frontend over
`/api/v1/ws/market`). Mark price is fully wired (parsed onto
`TickerEvent.mark_price`, stored in `MarketStateManager`, and actually
serialized in the WebSocket gateway's own outbound ticker payload,
`app/marketdata/gateway.py`'s `_ticker_payload`). Open interest is parsed
and stored (`TickerEvent.open_interest`) but silently dropped from
`_ticker_payload` — never reaches any API consumer or the frontend, a
real but narrow wiring gap, not a rebuild. Funding rate is the one
genuinely unbuilt item: `FundingRateEvent` and its `"funding_rate"`
channel dispatch registration exist at the low WebSocket-parser level
(`app/integrations/delta/websocket/`), but `LIVE_CHANNELS` never
subscribes to that channel, so nothing downstream — pipeline, event bus,
state manager, or any API — ever sees a real funding rate value. The
liquidation heatmap has zero existing code or documentation reference
anywhere in this repository (confirmed by a repo-wide search) — deferred,
not started, per `M4-E2-T2`.
