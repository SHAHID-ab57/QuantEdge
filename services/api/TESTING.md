...
(`base`, `errors`, `registry`, `fear_greed`), `app/models/external_data.py`,
`app/repositories/external_data.py`, `app/services/external_data_ingest.py`
and `external_data_sync.py` (structured line-for-line after
`candle_ingest.py`/`candle_sync.py`, and tested the same way, against a
`FakeConnector` test double rather than the real network),
`app/services/external_data_context.py` (the train/serve-skew-prevention
helper shared by `FeatureService` and `MLDatasetService`), the
connector-backed feature extension to
`app/features/base.py`/`pipeline.py`/`dataset.py` proved via the first
concrete generator, `app/features/builtin/fear_greed.py`, and the
registry-driven read API backing the Data Sources page (M4-E1-T3),
`app/services/connectors.py`/`app/api/v1/endpoints/connectors.py`.
