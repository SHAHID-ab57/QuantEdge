...
## Testing External Data Connectors

Covers `ARCHITECTURE.md` § "External Data Connectors" — `app/connectors/`
(`base`, `errors`, `registry`, `fear_greed`, `fred`),
`app/models/external_data.py`, `app/repositories/external_data.py`,
`app/services/external_data_ingest.py` and `external_data_sync.py`
(structured line-for-line after `candle_ingest.py`/`candle_sync.py`, and
tested the same way, against a `FakeConnector` test double rather than
the real network), `app/services/external_data_context.py` (the
train/serve-skew-prevention helper shared by `FeatureService` and
`MLDatasetService`), the connector-backed feature extension to
`app/features/base.py`/`pipeline.py`/`dataset.py` proved via two concrete
generators, `app/features/builtin/fear_greed.py` and
`fed_funds_rate.py`, and the registry-driven read API backing the Data
Sources page (M4-E1-T3), `app/services/connectors.py`/`app/api/v1
/endpoints/connectors.py`.

| File                                                                                                                         | Covers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/connectors/test_fear_greed.py`                                                                                        | `TestFearGreedClient` — mirrors `tests/unit/delta/test_client.py`'s own `httpx.MockTransport` conventions exactly: a successful fetch against the real alternative.me response shape (string `value`/`timestamp` fields, verified rather than assumed numeric); the `limit=0` full-history sentinel; three distinct malformed-response shapes (non-JSON body, missing `data` field, a non-object envelope) each raising `ConnectorAPIError`; the API's own in-band `metadata.error` field raising the same; an HTTP error status; a network timeout and a connect-error-retried-then-exhausted both raising `ConnectorNetworkError`; and a 429 retried-then-succeeds followed by a 429-exhausted-raises-`ConnectorRateLimitError`, proving the bounded exponential-backoff retry loop is real, executed code. `TestFearGreedConnector` — `fetch()` maps the raw response into `RawDataPoint`s and filters to the requested `[start, end]` range (proven by a fixture entry deliberately outside the requested window being excluded, not merely absent from a narrower source response); two malformed-entry shapes (non-numeric `value`, a missing key) each raise `ConnectorAPIError`; a naive (non-timezone-aware) `start`/`end` and `end` before `start` are both rejected before any request is made.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `tests/connectors/test_registry.py`                                                                                          | `ConnectorRegistry` mirrors `FeatureRegistry`'s own test shape: register/get/duplicate-rejection/not-found, `describe()`/`describe_all()`, `names()`/`has()`/`len()`. `TestFearGreedIsRegistered` confirms `fear_greed` is discoverable on the real, shared `default_registry` — not just an isolated test registry — the same "is it really wired into the real app" proof `test_pipeline.py`'s own builtin-discovery tests use.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `tests/features/test_fear_greed_feature.py`                                                                                  | `TestDiscovery` — `fear_greed` appears in `FeaturePipeline.describe_all()` with `external_sources == ("fear_greed",)` and `missing_values_expected is True`, with no change to the registry or pipeline needed. `TestGenerate` — the most-recent-value-at-or-before lookup across mixed timestamps, an exact-timestamp match counting as "at or before," `None` before the earliest recorded value (never a fabricated 0 or a forward fill), and `None` for every row when no external data was pre-fetched at all. `TestNoLookAhead` — the adversarial proof this task explicitly required: a fake data point dated _after_ every candle in the series is added, and the past computation must come back byte-for-byte identical. Proven at three levels: the pure in-memory generator (future point last, then again with the same point listed _first_ in the input to confirm the bisect search itself enforces this, not incidental list order); and `test_end_to_end_through_the_real_ingested_data_path`, which computes the feature once, then _actually inserts_ one more row into the real `external_data_points` table via `ExternalDataRepository` and recomputes through the full `resolve_external_data` → `FeaturePipeline.run` path — the same guarantee proven through the real database-backed pre-fetch, not just the isolated helper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `tests/features/test_pipeline.py::TestCache::test_a_generator_declaring_external_sources_never_hits_even_with_a_cache_wired` | The never-cached design decision itself (`ARCHITECTURE.md` § "External Data Connectors": `FeatureCacheKey` fingerprints candles and parameters only, never a connector's own freshness), proven against a real, active `FeatureCache` — not merely the pre-existing "no cache configured at all" case. A generator declaring `external_sources` reports `cache_status == "disabled"` on _every_ call, and its own call counter (the same instrumentation pattern `Counting` uses to prove a genuine hit) shows `generate()` ran every time, never skipped.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `tests/api/test_features_api.py::TestCatalogueEndpoint::test_publishes_external_sources_for_a_connector_backed_feature`      | `external_sources` over real HTTP, not just the internal dataclass — `GET /api/v1/features` reports `["fear_greed"]` for `fear_greed` and `[]` for an ordinary candle-only generator (`ohlcv`), and `GET /api/v1/features/fear_greed` reports the same. **Caught a real bug**: `FeatureMetadata.external_sources` was read internally (the cache-bypass check) but never threaded onto `FeatureDTO` — the Pydantic model this endpoint actually serializes — so the real running dev server's own `GET /api/v1/features` silently omitted the field for every generator despite it already being documented as part of the response. Found by querying the real, already-running dev server directly and comparing against the documented shape, not by a test (no test had exercised the DTO layer for this field at all); fixed in `app/schemas/features.py`, re-verified live afterward.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `tests/services/test_external_data_ingest.py`                                                                                | `ingest_external_data`/`_persist_points`, against a `FakeConnector` test double (never the real network — that is `tests/connectors/test_fear_greed.py`'s job) and a real in-memory SQLite session, mirroring `tests/services/test_candle_ingest.py`'s own shape: a successful fetch persists every point; re-ingesting the identical range inserts nothing the second time (`duplicates_skipped`, not a second row); a connector's own `ConnectorNetworkError` propagates with nothing persisted; an unregistered source and a naive or reversed range are all rejected before any fetch is attempted. `test_database_conflict_is_isolated_and_rejected` forces the database-level savepoint backstop past the application-layer pre-check (mirroring the candle version's own equivalent test) using a non-`None` symbol — and documents, in the same test, why this specific backstop can never fire for Fear & Greed itself (`symbol` always `None`; Postgres/SQLite both treat two `NULL`s as distinct for uniqueness), leaving the application-layer check as Fear & Greed's _only_ line of defense. **Caught a real, previously-untested bug while being written**: `_persist_points` mixed an explicit `session.begin()` with a query that had already auto-begun a transaction (`InvalidRequestError`), fixed by relying on the session's own autobegin instead; separately, `ExternalDataRepository.list_existing_timestamps` was returning _naive_ datetimes on SQLite (the same round-trip quirk `candle_sync.py`/`external_data_context.py` already work around elsewhere) while freshly-fetched points are always timezone-aware — silently failing every `in`-based duplicate check on SQLite specifically (no exception, just a wrong answer), fixed with the same `_as_utc()` normalization pattern used everywhere else this quirk appears.                                                                                                                 |
| `tests/services/test_external_data_sync.py`                                                                                  | `ExternalDataSyncScheduler`, mirroring `tests/services/test_candle_sync.py`'s own shape (start/stop, `_catch_up_window` computation, `run_catch_up` fan-out, background loop): no-op without a configured database; a source with nothing stored yet seeds from `backfill_days`; a source with a stored point resumes the window _at_ (not after) that point's own timestamp — this table's deliberately inclusive-both-ends convention, unlike `Candle`'s half-open buckets, so the boundary point is safely re-fetched and skipped as a duplicate rather than risking a gap; an already-current source is skipped; every configured, registered source is synced independently; an unregistered source is excluded without failing the run; one source's failure never stops the rest; the background loop starts, runs a tick, and stops cleanly; an empty source/registry combination is a clean no-op, never an error.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `tests/unit/runtime/test_runtime.py`                                                                                         | `ExternalDataSyncScheduler` wiring mirrors every other scheduler's own runtime tests: absent when `external_data_sync_enabled` is `false` or no database is configured; `test_runtime_shutdown_stops_external_data_sync` confirms `shutdown()` stops it cleanly alongside the candle sync scheduler.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `tests/api/test_connectors_api.py`                                                                                           | The Data Sources page's own backend (M4-E1-T3), over real HTTP against the in-memory SQLite database, mirroring `test_features_api.py`'s own shape. `TestConnectorCatalogueEndpoint`: `fear_greed` listed; a registered-but-never-ingested connector reports `null` `latest_value`/`latest_timestamp`, never a crash; the real latest value once a point is seeded directly; a zero-registered-connectors catalogue (the registry monkeypatched to an empty `ConnectorRegistry`) still returns a clean, empty response rather than an error; mounted unversioned too. `TestConnectorHistoryEndpoint`: points returned within an inclusive `[start, end]` range (this table's own convention, not candles' half-open one); omitting both `start` and `end` queries all stored history; `limit`/`offset` pagination proven across two real pages (`has_more` correctly `true` then `false`); an unknown `source` 404s `connector_not_found` (naming the one real registered source in its own detail message); mounted unversioned too. `test_a_newly_registered_connector_appears_with_zero_code_change` confirms `fed_funds_rate` (added well after this endpoint) appears with no change here — the same proof `test_features_api.py` gives `/features`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `tests/connectors/test_fred.py`                                                                                              | Mirrors `test_fear_greed.py`'s own coverage shape (successful fetch, every malformed-response case, network failure, rate-limit retry/exhaustion) plus what's genuinely new to this connector — the first requiring authentication. `TestFredClient`: no API key configured raises `ConnectorAuthenticationError` with no network call at all; a bad/unregistered key (FRED's own `error_message` mentioning `api_key`, per its real error envelope `{"error_code", "error_message"}`) raises the same; a 400 for an unrelated reason (an unknown series id) raises `ConnectorAPIError`, proven **not** to be misreported as an authentication failure. `TestFredClient::test_successful_fetch_returns_the_raw_observations_array` proves the two params load-bearing for this whole design (`realtime_start=1776-07-04`, `realtime_end=9999-12-31`) are actually on every real request — added after a real bug this exact gap let through: an earlier version omitted them, verified only against a mocked response that hand-picked distinct `realtime_start` values per entry, and the real backfill against the live API immediately exposed it (865 of 866 real historical values silently discarded as false in-batch duplicates, all collapsed onto today's date). Fixed and re-verified against the real API, not just re-mocked — see `ARCHITECTURE.md` § "FRED Connector" for the full account. `TestFredConnector::test_fetch_uses_realtime_start_never_the_reference_date` — the load-bearing proof: a fixture where August's own average is dated `"2026-08-01"` but published `"2026-09-01"` must be timestamped by the latter; `test_fetch_excludes_points_outside_the_requested_publication_range` confirms the exclusion is genuinely `realtime_start`-based, not merely absent from a narrower source response; `test_a_missing_value_sentinel_is_skipped_not_fabricated` — FRED's own literal `"."` for "no value yet" is skipped, never parsed as `0.0`. |
| `tests/connectors/test_registry.py::TestFredIsRegistered`                                                                    | Confirms `fed_funds_rate` is discoverable on the real, shared `default_registry` (`requires_auth is True`, `frequency == "monthly"`) — the same "is it really wired into the real app" proof `TestFearGreedIsRegistered` already gives.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `tests/features/test_fed_funds_rate_feature.py`                                                                              | Mirrors `test_fear_greed_feature.py`'s own `TestDiscovery`/`TestGenerate`/`TestNoLookAhead` shape exactly. `TestPublicationLagFixture::test_a_candle_mid_reference_month_sees_no_value_yet` is the one genuinely new class: a real mock FRED response (August, dated `2026-08-01`, published `2026-09-01`) is run through the actual `FredConnector` and real ingestion (`ExternalDataRepository`, not a hand-picked `ExternalDataPoint` construction), then a candle at `2026-08-15` (after the reference date, before the real publication date) is proven to see `null`, while one at `2026-09-02` sees the real value — the fixture test this task's own investigation explicitly required, proven end to end through the real connector's own mapping rather than asserted against a synthetic timestamp.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

**The no-look-ahead adversarial test is the load-bearing proof for every
future connector, not just this one.** Any connector-backed feature added
later should carry the same three-level test (pure generator, out-of-order
input, real-database-backed end-to-end) before being trusted — a connector
whose feature silently used a value published after the candle it describes
would be invisible in ordinary testing (values still look plausible) and
would only surface as inflated backtest/live-prediction accuracy, much
later and much harder to trace back to its actual cause.

**Naive-vs-aware datetime handling is proactively covered, not
incidentally.** `external_data_context.py`'s `_as_utc()` helper (mirroring
the same pre-existing SQLite-quirk workaround in `candle_sync.py`'s own
`_catch_up_window`) is exercised by every `TestNoLookAhead` case that
round-trips through a real SQLite-backed session — a regression here would
surface as `TypeError: can't compare offset-naive and offset-aware
datetimes` inside `most_recent_value_at_or_before`, not a silent wrong
value, so this class of bug fails loudly rather than quietly.
