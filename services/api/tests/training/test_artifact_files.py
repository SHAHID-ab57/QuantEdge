...
# `_reports_directory()` (and every `write_*` function's own return value —
# see that module's docstring for the portability fix) always joins
# `reports/<filename>` onto `model_artifact_dir`'s bare directory name — not
# an absolute `file://` URI. `_reports_into_tmp_path` redirects the *directory
# name itself* to a per-test `tmp_path`, so `tmp_path / uri` reads the file
# straight back with no extra resolution step needed.
...
def _reports_into_tmp_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(artifact_files, "_reports_directory", lambda: tmp_path / "reports")


@pytest.fixture
def read(tmp_path: Path) -> Callable[[str], bytes]:
    def _read(uri: str) -> bytes:
        assert not uri.startswith("file://"), "expected a path relative to model_artifact_dir"
        return (tmp_path / uri).read_bytes()

    return _read


class TestWriteMetricsJson:
    def test_writes_a_readable_json_file(self, read: Callable[[str], bytes]) -> None:
        uri = artifact_files.write_metrics_json("logistic_regression", {"accuracy": 0.9})

        assert uri.endswith(".json")
        assert json.loads(read(uri)) == {"accuracy": 0.9}
...
class TestWriteTrainingReportJson:
    def test_writes_a_readable_json_file(self, read: Callable[[str], bytes]) -> None:
        ...
        assert json.loads(read(uri))["n_train"] == 10


class TestWriteFeatureImportanceCsv:
    def test_writes_a_header_and_one_row_per_feature(self, read: Callable[[str], bytes]) -> None:
        rows = [
            {
                "feature": "open",
                "coefficient": 0.5,
        ...
        lines = read(uri).decode().splitlines()
    ...
    def test_tolerates_a_row_missing_the_normalized_key(self, read: Callable[[str], bytes]) -> None:
...
class TestWriteConfusionMatrixPng:
    def test_writes_a_non_empty_png_file(self, read: Callable[[str], bytes]) -> None:
