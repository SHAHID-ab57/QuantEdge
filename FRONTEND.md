...
future connector (Marketaux, DefiLlama, CoinGecko) appears here the day
it's registered on the backend, with no frontend change — `fed_funds_rate`
and `eth_gas_price` already proved this guarantee once each. Each
`ConnectorCard` fetches its own history independently via
...
Marketaux, DefiLlama, CoinGecko — rendered by `PlannedDataSources` with
zero API call, since there is nothing in the registry for an unbuilt
connector to read. This list is **not** automatically kept in sync: an
entry must be removed the same day its connector actually ships (appears
in `GET /connectors`), never before — otherwise a source briefly shows as
...
not before and not after. Etherscan (M4-E1-T4) is the more ordinary case:
no prior "wrongly reported" history to record, just a plain removal from
this array the same day `app/connectors/etherscan.py` actually landed.
