...
Liquidation Heatmap Investigation), has its first task (`M4-E2-T1`) done
and its second (`M4-E2-T2`, the liquidation heatmap) still deferred —
see `M4-E2-T1`/`M4-E2-T2` below. M4-E1's first item, the reusable connector abstraction
...
tables accumulate real coherent rows on the configured cadence). The
liquidation heatmap (`M4-E2-T2`) still has zero existing code or
documentation reference anywhere in the repo (confirmed by a repo-wide
search) — deferred, not started.
